...
export const meetingSpeakingExercises: MeetingSpeakingExercise[] = [
  // === NIVEAU A2 ===
  {
    ...
    prompt:
      "Dites bonjour à l'équipe, remerciez-les d'être présents et annoncez l'objectif principal de la réunion.",
    targetPhrases: [
  ...
  },
  {
    ...
    tips: ["Montrez de l'empathie", "Posez des questions simples", "Confirmez votre compréhension"],
  },
  {
    id: "meeting_a2_3",
    level: "A2",
  ...
  // === NIVEAU B1 ===
  {
    ...
    prompt:
      "Proposez d'analyser les logs et suggérez que le problème pourrait venir de la dernière mise à jour.",
    targetPhrases: [
  ...
  },
  {
    ...
    prompt:
      "Expliquez que vous avez bien compris le problème et donnez le contexte technique : le problème est apparu après la mise à jour et affecte le module d'importation.",
    targetPhrases: [
  ...
  },
  {
    ...
    prompt:
      "Proposez trois options : redémarrer le service, déployer un correctif temporaire, ou revenir à la version précédente.",
    targetPhrases: [
  ...
  // === NIVEAU B2 ===
  {
    ...
    scenario:
      "Un collègue propose une solution que vous trouvez risquée. Vous devez exprimer votre désaccord de manière diplomatique.",
    prompt:
      "Exprimez que vous comprenez son point de vue, mais proposez une approche alternative plus sûre en expliquant les risques potentiels.",
    targetPhrases: [
  ...
  },
  {
    ...
    prompt:
      "Expliquez l'architecture actuelle, identifiez le goulot d'étranglement (bottleneck), et proposez une solution de scaling horizontal avec load balancing.",
    targetPhrases: [
  ...
  },
  {
    ...
    scenario:
      "Le système est inaccessible pour certains utilisateurs. Vous devez mener l'investigation.",
    prompt:
      "Analysez la situation : vérifiez les logs, testez la connectivité réseau, identifiez les utilisateurs affectés, et proposez un plan d'action avec des priorités.",
    targetPhrases: [
  ...
  // === NIVEAU C1 ===
  {
    ...
    scenario:
      "Vous animez une réunion pour décider de la migration vers une architecture microservices. Il y a des opinions divergentes.",
    prompt:
      "Facilitez la discussion : présentez les enjeux, donnez la parole à chaque partie, synthétisez les arguments, et guidez l'équipe vers un consensus en tenant compte des contraintes techniques, budgétaires et de timeline.",
    targetPhrases: [
  ...
  },
  {
    ...
    scenario:
      "Un client majeur menace de résilier le contrat suite à des problèmes de performance récurrents. Vous devez gérer la situation.",
    prompt:
      "Menez la réunion de crise : reconnaissez les problèmes rencontrés, présentez l'analyse des causes profondes, détaillez le plan d'action correctif avec timelines, proposez des compensations, et restaurez la confiance.",
    targetPhrases: [
  ...
  },
  {
    ...
    scenario:
      "Vous devez convaincre les stakeholders (CTO, CFO, Product Manager) d'investir dans une plateforme MLOps.",
    prompt:
      "Présentez la stratégie : contexte business, problèmes actuels (dette technique ML, time-to-market, reproductibilité), bénéfices attendus (ROI, scalabilité, gouvernance), architecture proposée, roadmap de déploiement, et budget. Gérez les questions et objections.",
    targetPhrases: [
  ...
  },
  {
    ...
    scenario:
      "Suite à une panne de 4 heures affectant tous les utilisateurs, vous animez la réunion de post-mortem avec toutes les équipes impliquées.",
    prompt:
      "Facilitez le post-mortem : établissez un timeline des événements, identifiez les causes techniques et organisationnelles, analysez pourquoi les mécanismes de prévention ont échoué, définissez des actions correctives avec responsables et deadlines, et créez une culture d'apprentissage sans blâme.",
    targetPhrases: [
