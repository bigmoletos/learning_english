...
const mapGoogleVoiceToBrowserVoice = (
  ...
  console.log(
    "[GoogleTTS] Liste complète des voix:",
    voices.map((v) => ({
      name: v.name,
      lang: v.lang,
  ...
  if (isUS) {
    ...
    const usVoices = voices.filter(
      (v) => v.lang.startsWith("en-US") && !v.name.toLowerCase().includes("french")
    );

    console.log(
      "[GoogleTTS] Voix US disponibles:",
      usVoices.map((v) => v.name)
    );
    ...
    if (isFemale) {
      ...
      if (!selectedVoice) {
        const femaleCandidates = usVoices.filter(
          (v) =>
    ...
    const gbVoices = voices.filter(
      (v) => v.lang.startsWith("en-GB") && !v.name.toLowerCase().includes("french")
    );
    ...
    if (isFemale) {
      ...
      selectedVoice =
        ...
        gbVoices.find(
          (v) => !v.name.toLowerCase().includes("male") && !v.name.toLowerCase().includes("george")
        ) ??
        null;
    } else if (isMale) {
      ...
      selectedVoice =
        ...
        gbVoices.find((v) => v.name.toLowerCase().includes("male")) ??
        null;
    }
  ...
  if (!selectedVoice) {
    const englishVoices = voices.filter(
      (v) => v.lang.startsWith("en-") && !v.name.toLowerCase().includes("french")
    );
...
export const useGoogleTTS = (): UseGoogleTTSReturn => {
    ...
    const checkGoogleTTS = async () => {
      try {
        ...
        const isAvailable: boolean =
          response.ok && contentType !== null && contentType.includes("application/json");
        ...
        if (!isAvailable) {
          ...
          console.log(
            "[GoogleTTS] Note: Les voix seront différenciées par pitch et rate même sans Google Cloud TTS"
          );
        } else {
        ...
        console.warn(
          "[GoogleTTS] Impossible de contacter le backend, fallback vers Web Speech API",
          err.message
        );
        console.log(
          "[GoogleTTS] Note: Les voix seront différenciées par pitch et rate même sans Google Cloud TTS"
  ...
  const speak = useCallback(
    async (text: string, lang = "en-US", voice?: string) => {
      ...
      try {
        if (isGoogleTTSAvailable) {
          ...
          console.log(
            "[GoogleTTS] Audio généré avec succès, URL:",
          ...
          if (voice) {
            ...
            const selectedVoice =
              voice.includes("Neural2") || voice.includes("en-")
            ...
            if (selectedVoice) {
              ...
              const isNeuralVoice =
                voice &&
                typeof voice === "string" &&
                (voice.includes("Neural") || voice.includes("en-US") || voice.includes("en-GB"));
              console.log("[GoogleTTS] Vérification voix Neural:", {
                voice,
                isNeuralVoice,
                type: typeof voice,
              });
              ...
              if (isNeuralVoice) {
                ...
                console.warn(
                  "[GoogleTTS] ⚠️ Voix non-Neural, pas d'ajustement de pitch/rate:",
                  voice
                );
              }
            } else {
